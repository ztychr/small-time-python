...
while True:
        try:
                ...
                random_id = ''.join(random.choice(letters) for i in range(32))

                username=random.choice(top500)
                password=random.choice(top500)
                sec_lev=random.randint(1,3)

                url = "https://"+host+":"+port+"/bWAPP/login.php"

                obj = {"login":username, "password":password, "PHPSESSID": random_id, "security_level":sec_lev, "form":"submit"}

                p = requests.post(url, verify=False, data=obj, cookies = {"PHPSESSID": random_id}, headers = { "HTTP-HOST":"192.168.56.3:8443"})
